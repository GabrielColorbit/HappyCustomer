...
myControllers.controller('ContatoController', function($scope, $routeParams,$http) {
	...
	$scope.EnviarInformacao = function() {
		...
		var parameter = JSON.stringify({
			type : "contato",
			id : $scope.contato.id,
			razao_social : $scope.contato.razao_social,
			cnpj : $scope.contato.cnpj,
			inscricaoestadual : $scope.contato.inscricaoestadual,
			site : $scope.contato.site,
			ramo : $scope.contato.ramo,
			tipocontato : $scope.contato.tipocontato,
			cidade : $scope.contato.cidade,
			telefones: $scope.contato.telefones,
		...
		$http.post(
				...
				function(data, status, headers, config) {
					$scope.Resposta = 'Contato '+$scope.contato.razao_social+' Salva com Sucesso!';
